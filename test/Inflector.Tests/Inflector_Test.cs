...
namespace Inflector.Tests
{
    public class Inflector_Test
    {
        ...
        public Inflector_Test()
        {
            _service = PluralizationService.CreateService(new System.Globalization.CultureInfo("en"));
        }
        ...
        [Fact]
        public void Pluralize_Test()
        {
            foreach (var kv in TestData)
            {
                Assert.Matches(_service.Pluralize(kv.Key), kv.Value);
            }

        }

        [Fact]
        public void Singularize_Test()
        {
            foreach (var kv in TestData)
            {
                Assert.Matches(_service.Singularize(kv.Value), kv.Key);
            }
        }
    }
}
